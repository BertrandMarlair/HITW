...
function serverUsage(api, server) {
  ...
  return api.runSSHCommand(server, 'df -h /')
    ...
    .then(() => {
      console.log(chalk.cyan('=> Apps, MongoDB Config, and Reverse Proxy'));
    ...
    .then(() => {
      console.log(chalk.cyan('=> MongoDB Data'));
      return api.runSSHCommand(server, 'du -h --max-depth=0 /var/lib/mongodb');
    })
    .then((result) => {
      console.log(result.output);
    })
...
function show(api) {
  ...
  Promise.each(Object.values(servers), server => serverUsage(api, server));
}
